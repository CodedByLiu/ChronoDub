...
- 使用 DeepSeek 进行默认单策略的 segment 级英译中翻译，并按分段文本预算约束结果长度。
...
1. 在配置面板先设置 `DeepSeek API Key`、`输出目录`、`音色`。
2. `最大并发任务数`先用 `2`。
3. 先用 1-3 分钟短视频验证一次自动识别与成片效果。
4. 确认效果后，再批量处理长视频。
...
- `复核模式`
  ...
  - `手动复核`：必须显式确认后才继续配音。
- `翻译策略`
  - 当前为内置默认单策略，不再区分快速/平衡/极致模式。
  - 系统会自动应用分段文本预算约束，减少超窗与过长译文。
- `字幕输出方式`
  - `生成字幕文件`：输出配音视频和 `.srt`。
  - `硬嵌字幕`：输出一个带中文字幕的成品视频。
- `硬嵌字幕样式`
...
- 长视频建议优先把并发降到 `2`（最稳妥起点）。
- 若 Windows 下出现 Electron GPU 进程崩溃，通常是系统/驱动渲染问题，不是字幕逻辑问题。建议降低并发，并避免同时运行多个重 GPU 应用。
- 如果任务被系统睡眠中断，可在重新打开应用后从任务列表继续执行。
- 如果某个视频的硬嵌字幕样式看起来异常，请先检查视频元数据；当前版本会按检测到的实际显示尺寸和旋转信息缩放字幕布局。
- 如果在输出阶段取消或失败，当前版本会自动清理半成品文件，减少损坏成品残留。
