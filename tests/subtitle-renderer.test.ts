...
test('renderAssSubtitles emits bottom-safe subtitle style', () => {
  ...
  assert.match(ass, /Style: Default,Arial,48,/)
  assert.match(ass, /,2,120,120,72,1/)
  assert.match(ass, /Dialogue: 0,0:00:00\.00,0:00:01\.50,Default,,0,0,0,,第一行字幕/)
})
...
test('renderAssSubtitles switches to top-safe alignment and background box', () => {
  ...
  assert.match(ass, /,4,3,4,8,120,120,72,1/)
  assert.match(ass, /&H00000000,&H6600A5FF,0,0,0,0,100,100,0,0,4,3,4,8/)
})
...
test('renderAssSubtitles writes bold and italic flags when enabled', () => {
  ...
  assert.match(ass, /,&H00000000,-1,-1,0,0,100,100,0,0,1,3,0,2,/)
})
