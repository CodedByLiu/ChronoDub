...
const EN_REAL_WORD_RE = /[A-Za-z]{3,}/
...
export function looksLikeSentence(text: string): boolean {
  ...
  if (!EN_REAL_WORD_RE.test(text)) return false
...
export function shouldRetryAsOverCompressed(
  source: string,
  translated: string,
  maxChars?: number
): boolean {
  ...
  if (typeof maxChars === 'number' && maxChars > 0) {
    const budgetMin = Math.max(2, Math.ceil(maxChars * 0.3))
    if (dstChars >= budgetMin) return false
  }
...
export function isAcceptableTranslatedText(
  source: string,
  translated: string,
  maxChars?: number
): boolean {
  ...
  if (shouldRetryAsOverCompressed(source, normalized, maxChars)) return false
  return true
}
